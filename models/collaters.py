#TODO: Daniel replaces D3PMCollater with new class called ContinuousCollater to do the noising.
# NOS-C noise scheduler will be useful: https://github.com/ngruver/NOS/blob/569f1c85adf2ca2ea5e32efdf46276995fcf322c/seq_models/schedule/noise_schedule.py#L114 #
...
class ContinuousCollater(object):
    ...
    A simplified collater for continuous (Gaussian) diffusion that:
      1) Tokenizes each sequence into integer IDs.
      2) Pads them to the same length.
      3) Creates:
         - attn_mask: 1 = real token, 0 = pad
         - corrupt_mask: 1 = "noise this token", 0 = "keep token"
           (Here we set corrupt_mask=1 for all real tokens, mimicking the discrete
            collater's logic of "fully corrupt" by default.)
      4) Returns a dictionary consistent with your continuous model's `forward`:
         {
           "seq": (B, L) int IDs,
           "attn_mask": (B, L) 1=real,0=pad,
           "corrupt_mask": (B, L) 1=noise,0=keep,
           "labels": None   # if you want regression labels, attach them here
         }
    No special tokens or partial corruption is handled here.
    """
    ...
    def __call__(self, sequences):
        """
        `sequences` is a list of tuples, e.g. [(seq_str,), (seq_str,), ...]
        coming from your SequenceDataset.
        """
        ...
        for i, seq_tuple in enumerate(sequences):
            seq_str = seq_tuple[0]
            ids = self.tokenizer.tokenize(seq_str)
            if len(ids) == 0:
        ...
        # Edge case: if all are empty, return an empty batch
        if len(tokenized) == 0:
            return {
        ...
        padded = _pad(tokenized, value=self.pad_id)  # shape (B, L)
        batch_size, max_len = padded.shape

        # 3) attn_mask: 1 if != pad, else 0
        attn_mask = (padded != self.pad_id).long()

        # 4) corrupt_mask: 1 for real tokens, 0 for pad
        #    (Equivalent to "fully corrupt all real tokens")
        corrupt_mask = attn_mask.clone()  # (B, L), 1=corrupt, 0=pad

        # 5) Return dictionary. No timesteps (the model samples them),
        #    no one-hot, no Q, etc. The model will do the actual noising.
        return {
